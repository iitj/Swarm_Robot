//Ball Detection Algorithm 3
//Date - 30/10/2011 
/*This is different from the previous code. This has my own addition
Now I am comparing the radius of the circle with the previous.If the variation is too much, the circle is discarded. The noise is considerable reduced
But the ball is not being effectively detected many times. The code works fine, but this is completly experimental. I had to do a lot of chages to get this working
*/
#include <stdio.h>
#include <stdlib.h>
#include <cv.h>
...
*/
      ...
      for(i = 0;i<circles->total;i++)
	{
	  ...
	  if(rpre == 0 && ypre == 0 && xpre == 0)
	    ...
	    {
	      if((abs(p[2] - rpre) < 40) && ((abs(p[0] - xpre) < 40) || (abs(p[1] - ypre) < 40)))
		{
		  cvCircle( frame, cvPoint(cvRound(p[0]),cvRound(p[1])), 3, CV_RGB(0,255,0), -1, 8, 0 );
		  cvCircle( frame, cvPoint(cvRound(p[0]),cvRound(p[1])), cvRound(p[2]), CV_RGB(255,0,0), 3, 8, 0 );
		  printf("Ball! x=%f y=%f r=%f\n\r",p[0],p[1],p[2] );
		  printf("Previous Ball Values x=%f y=%f r=%f\n", xpre, ypre, rpre);
		}
	      else;
	    }
	  xpre = p[0];
